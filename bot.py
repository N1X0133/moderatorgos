...
# БЕЛЫЙ СПИСОК СЕРВЕРОВ (ТОЛЬКО НА ЭТИХ СЕРВЕРАХ БОТ БУДЕТ РАБОТАТЬ)
ALLOWED_GUILD_IDS = [
    806135792421699584,
    768174465745813531,
    886219875452854292,
...
ADDITIONAL_LEADERS_CHANNEL_ID = 1481754087614841033  # Дополнительный канал для команд !лидеры и !кураторы
...
class ModerationBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        print(f"   • Серверов в белом списке: {len(ALLOWED_GUILD_IDS)}")
        print()
...
# ==================== ПРОВЕРКА ПРАВ И СЕРВЕРА ====================

def is_allowed_guild():
    """Проверка, что команда выполняется на разрешенном сервере"""
    async def predicate(interaction: discord.Interaction):
        if interaction.guild_id in ALLOWED_GUILD_IDS:
            return True
        raise app_commands.errors.CheckFailure("Этот сервер не в белом списке!")
...
def combined_checks():
    """Комбинированная проверка: сервер в белом списке + пользователь админ"""
    async def predicate(interaction: discord.Interaction):
...
async def sync_command(ctx):
    if ctx.guild.id not in ALLOWED_GUILD_IDS:
        await ctx.send("❌ Этот сервер не в белом списке!")
        return
...
async def leaders_command(ctx, *, текст: str = None):
    ...
    # Проверка сервера (только разрешенные сервера)
    if ctx.guild.id not in ALLOWED_GUILD_IDS:
        await ctx.send("❌ Этот сервер не в белом списке!")
        return
    ...
    if ctx.channel.id not in [LEADERS_CHANNEL_ID, ADDITIONAL_LEADERS_CHANNEL_ID]:
        await ctx.send(f"❌ Эту команду можно использовать только в каналах для списков!")
        return
    
    # Получаем канал для лидеров (основной канал, куда отправляем сообщение)
...
async def curators_command(ctx, *, текст: str = None):
    ...
    # Проверяем, что команда используется в разрешенном канале
    if ctx.channel.id not in [CURATOR_CHANNEL_ID, ADDITIONAL_LEADERS_CHANNEL_ID]:
        await ctx.send(f"❌ Эту команду можно использовать только в каналах для списков!")
        return
    
    # Получаем канал для кураторов (основной канал, куда отправляем сообщение)
...
async def on_guild_join(guild):
    """Проверка при добавлении бота на сервер"""
    if guild.id not in ALLOWED_GUILD_IDS:
        logger.warning(f"⚠️ Бот добавлен на неразрешенный сервер: {guild.name} (ID: {guild.id})")
        # Пытаемся покинуть сервер
        try:
            owner = guild.owner
            if owner:
                await owner.send(f"❌ Бот не может работать на сервере **{guild.name}**, так как он не в белом списке разрешенных серверов.")
        except:
            pass
        await guild.leave()
        return
    
    logger.info(f"✅ Бот добавлен на разрешенный сервер: {guild.name} (ID: {guild.id})")
...
async def on_message_edit(before, after):
    if before.guild and before.guild.id not in ALLOWED_GUILD_IDS:
        return
...
async def on_bulk_message_delete(messages):
    if not messages:
        return
    guild = messages[0].guild
    if guild and guild.id not in ALLOWED_GUILD_IDS:
        return
